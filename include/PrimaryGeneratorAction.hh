...
/// The primary generator action class with particle gum.
    ...
    public:
        ...
        void GenerateLowQ_ep_plus_ed_method2(G4Event* event); // for ep- (elastic, unpolarized) + ed-generator (elastic, polarized)  for LQ-polarimeter
...
//////// FOR LQ-generator
 ...
 G4double dsdo_LQ_ed_elastic(G4double theta_e, G4double Pzz, G4double zz_cell) {
            ...
            G4double Mt = (pow(alpha_em * Fm, 2.) * 10000. / 4.) * (1. - pow(beta_electron,2.) * pow(sin(theta_e / 2.), 2.)) * S / (rc * pow(sin(theta_e / 2.), 4.) * pow(Ebeam, 2.));
       ...
       G4double dsdo_LQ_ep_elastic(G4double theta_e, G4double zz_cell)
        ...
        G4double dsdo_LQ_ep_quasi_elastic(G4LorentzVector &Pelectron_initial, G4LorentzVector &Pelectron_final,G4LorentzVector &Pproton_initial, G4LorentzVector &Pproton_final, G4double theta_e, G4double phi_e, G4double zz_cell)
        {
...
/////
           ...
           // Pelectron_initial.setE(Eelectron_initial + Me); // полная энергия
            ...
            G4double distr_bonn_potential[101] = {
                ...
                0.9665918, 0.9704258,     0.9737193,     0.9765596, 0.9790186, 0.9811566,
            0.9830235, 0.9846612, 0.9861036, 0.9873803, 0.9885156,
            0.9895294, 0.9904380, 0.9912565,     0.9919961,     0.9926671, 0.9932781, 0.9938356, 0.9943457, 0.9948140, 0.9952440,
            0.9956399, 0.9960047,     0.9963413,     0.9966521, 0.9969389, 0.9972037, 0.9974484, 0.9976741, 0.9978822,
            0.9980739, 0.9982503,     0.9984124,     0.9985616, 0.9986980, 0.9988228, 0.9989371, 0.9990408, 0.9991356,
            0.9992215, 0.9992995,     0.9993696,     0.9994332, 0.9994900, 0.9995410, 0.9995866, 0.9996274, 0.9996639,
            0.9996958, 0.9997242,     0.9997495,     0.9997714, 0.9997910, 0.9998078, 0.9998228, 0.9998358, 0.9998475,
            0.9998578, 0.9998671,     0.9998750,     0.9998825, 0.9998896, 0.9998960, 0.9999021, 0.9999081, 0.9999138,
          ...
          //  red_mass_n = Mn;
            ...
            // G4cout << PN_proton_in_deuteron->getX() << " !!! " << Pfermi*sin(Arnd)*cos(Brnd) << G4endl;
             ...
             // G4cout << PN_neutron_in_deuteron->getT() << " !!! " <<sqrt(Pfermi*Pfermi+red_mass_p*red_mass_p) << G4endl;
             ...
             *************************************************************
                ...
                G4double Sinv = red_mass_p*red_mass_p +2.*Eneutron_in_deu*Egamma-2.*PN_neutron_in_deuteron.z()*Egamma;
         ...
         ****/
            ...
            // Pelectron_initial, Pproton_initial `вычислены в ЛО где протон движется с импульсом Ферми, перейдем в систему где протон покоиться
         ...
         //   G4cout << "beta_vector = " << boost_vec << " gamma_z = " << gamma_z << " gamma = " << gamma <<G4endl;
          ...
          //  G4cout << " !!! P is REST e_i = " << Pelectron_rest_initial << " p_i = " << Pproton_rest_initial << G4endl;
           ...
           // G4cout << " !!! P is REST after rot e_i = " << Pelectron_rest_initial << " p_i = " << Pproton_rest_initial << G4endl;
            ...
            if (zz_cell >= 0) coeff_target = (l_zz_cell_temp / 2. - zz_cell) / pow(l_zz_cell_temp / 2., 2.);// коэффициент для учета труегольного распределения частиц по мишени
            if (zz_cell < 0) coeff_target = (l_zz_cell_temp / 2. + zz_cell) / pow(l_zz_cell_temp / 2., 2.);
           ...
           // G4double Mt = (pow(alpha_em * Fm, 2.) * 10000. / 4.) * (1. - pow(beta_electron,2.) * pow(sin(theta_e / 2.), 2.)) * S / (rc * pow(sin(theta_e / 2.), 4.) * pow(Eelectron_initial, 2.));
            // Mott cross-Section , mkb

           //  G4cout << "Mt = " << Mt << G4endl;

            G4double Mt = (pow(alpha_em * Fm, 2.) * 10000. * 4.) * (1. - pow(beta_electron,2.) * pow(sin(theta_e / 2.), 2.)) * S * pow(Eelectron_initial, 2.) / (pow(rc, 3.) * pow(Q2, 2.));
           ...
           //  G4cout << "Mt = " << Mt << G4endl;
            ...
            G4double q3md = sqrt(pow(Ebeam, 2.) + pow(Eelectron_final, 2.) - 2. * Eelectron_initial * Eelectron_final * cos(theta_e));        // вектор передачи от начального электрона к конечному
            G4double cos_theta_otdachi = (pow(q3md, 2.) + pow(Eelectron_initial, 2.) - pow(Eelectron_final, 2.)) / (2. * q3md * Eelectron_initial);// косинус угла между векторами по теореме косинусов
            G4double theta_otdachi = acos(cos_theta_otdachi);                                                 // угол дейтрона отдачи для формулы поляризованного сечения

            ////


          //  G4cout << "sqrt(Q2) = " << sqrt(Q2) << " " << -(Pelectron_rest_final-Pelectron_rest_initial).mag() << " " << -(Pelectron_final-Pelectron_initial).mag() << G4endl;
            ...
            //factor_cross_sec_to_lo = pow(1. + gamma_m*gamma_m + 2.*gamma_m*cos(theta_e), 3./2.) / abs(1. + gamma_m * cos(theta_e));
